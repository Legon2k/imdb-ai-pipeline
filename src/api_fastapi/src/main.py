...
async def trigger_scraping(chart: Literal["top", "moviemeter", "toptv", "tvmeter"] = "moviemeter"):
    ...
    with PodmanClient(base_url=CONTAINER_SOCKET_PATH) as client:
        client.containers.run(
            # Use the correct image name generated by Podman Compose
            image="localhost/imdb-ai-pipeline_scraper:latest",
            command=[f"--chart={chart}"],
            remove=False,  # Keep for debugging
            detach=True,
            # Use the special Podman host resolution flag
            # This maps host.containers.internal to the real host gateway IP
            network="imdb-ai-pipeline_internal_network",
            # FIX: Route traffic out to the host machine's published port
            environment={
                # "imdb_redis"  # Should resolve perfectly via Docker DNS
