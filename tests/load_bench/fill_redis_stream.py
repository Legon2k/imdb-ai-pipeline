...
def main():
    ...
    parser = argparse.ArgumentParser(description="Redis Stream Performance Bench Filler")
    parser.add_argument(
        "--host", 
        type=str, 
        default=None, 
    ...
    if env_path.exists():
        ...
        print(f"Warning: .env file not found at {env_path}. Using environment/default values.")
    ...
    print(f"Connecting to Redis at {redis_host}:{redis_port}, Target Stream: {stream_name}")
    ...
    print(f"Starting data ingestion of {total_messages} messages into Redis Stream '{stream_name}'...")
